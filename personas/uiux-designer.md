...
4. **목업 ≠ 실앱** — 색·강도·톤·세부 look 판단은 HTML 목업으로 *확정하지 않는다*. 목업(특히 비주얼 컴패니언)은 실제 UI와 다르게 보이는 경우가 잦다. 확정은 **실앱 live spike**(미커밋으로 실제 컴포넌트를 수정 → 실서버에서 1:1 확인 → 되돌리고 정식 코드로 구현)로 한다. 목업을 쓸 땐 프레임 기본 테마 대신 실제 토큰·폰트·CSS를 충실히 재현하고, 그래도 근사치로만 본다. 구조·레이아웃 탐색에는 목업도 충분하다.
