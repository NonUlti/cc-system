...
- **프로젝트 루트 경로**: 첫 번째 인자 (옵션이 아닌 경로 인자)
- **옵션**:
...
**경로가 인자로 지정되지 않은 경우**, 반드시 AskUserQuestion 도구로 프로젝트 경로를 물어본다:
- 질문: "CLAUDE.md를 생성할 프로젝트 경로를 입력해주세요."
- 옵션에 현재 작업 디렉토리를 포함하되, 사용자가 직접 경로를 입력할 수도 있도록 한다.
- 사용자가 답변한 경로를 프로젝트 루트로 사용한다.
